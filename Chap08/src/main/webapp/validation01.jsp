...
<head>
	...
	<script>
		...
		function checkNumber() {
			// form.age.value = '' 사용을 위해 지역변수로 선언
...
<body>
	<!-- onsubmit="return 여부"
		 - 여부 : true 이면, submit(요청전송)
		  		 false 이면, submit 안 함
	 -->
	<form name="joinForm" onsubmit="return checkJoin()" action="validation01_pro.jsp" method="post"  >
		<p>아이디 : <input type="text" name="id" maxlength="20" /> </p>
		<p>비밀번호 : <input type="password" name="pw" /> </p>
		<p>이름 : <input type="text" name="name" /> </p>
		<p>나이 : <input type="text" name="age" onkeypress="checkNumber()" /> </p>
